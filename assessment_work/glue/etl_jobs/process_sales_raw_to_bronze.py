...
csv_files = [f"s3://{bucket_name}/{obj['Key']}" for obj in objects.get("Contents", []) if obj["Key"].endswith(".csv")]
...
final_df.write.mode("overwrite").parquet(f"s3://{bucket_name}/{bronze_prefix}")

print("Data written to bronze zone.")
job.commit()
